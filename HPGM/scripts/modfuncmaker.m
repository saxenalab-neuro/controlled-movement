function [error, y] = modfuncmaker(numfeatures, ti, tf, Ts, setinitial, y_ic)
...
rt = linspace(ti, tf, numfeatures+1);
ind = floor(rt .* 1/Ts + 1);
%ind = find(rt==t);
rtdiffs = diff(rt);
...
for i=1:numfeatures
    ...
    if (features(i) == 0)
        ...
        y(ind(i):ind(i+1)) = ry(i); % Assign y values for output
    ...
    if (features(i) == 1)
        ...
        y(ind(i):ind(i+1)) = slope; % Assign y values
        continue % Go to next feature
    end
    ...
    if (features(i) == 2)
        ...
        y(ind(i):ind(i+1)) = tmpy; % Assign y values
        continue % Go to next feature
    end
